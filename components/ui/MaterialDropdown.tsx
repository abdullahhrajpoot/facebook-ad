...
export default function MaterialDropdown({
    ...
    return (
        <div className={`relative ${width} ${className} group/dropdown`} ref={dropdownRef}>
            {label && (
                <div className="text-[9px] sm:text-[10px] font-extrabold text-zinc-500 uppercase tracking-widest absolute -top-2 sm:-top-2.5 left-3 sm:left-4 bg-[#050505] px-1.5 sm:px-2 z-20 pointer-events-none group-focus-within/dropdown:text-white transition-colors">
                    {label}
                </div>
            )}
            ...
            <button
                ...
                className="w-full flex items-center justify-between bg-zinc-900/30 hover:bg-zinc-800/50 border border-white/10 text-white py-2.5 sm:py-3 md:py-4 px-3 sm:px-5 rounded-xl sm:rounded-2xl transition-all shadow-sm backdrop-blur-xl hover:border-white/20 active:scale-[0.99] h-[52px] sm:h-[64px]"
            >
                <div className="flex items-center gap-2 sm:gap-3 truncate">
                    {Icon && <Icon className="w-4 h-4 sm:w-5 sm:h-5 text-zinc-600 group-hover:text-zinc-400 transition-colors" />}
                    <span className={`font-bold text-xs sm:text-sm md:text-base truncate ${!selectedOption ? 'text-zinc-500' : 'text-zinc-200'}`}>
                        {selectedOption?.label || placeholder}
                    </span>
                </div>
                <ChevronDown className={`w-4 h-4 sm:w-5 sm:h-5 text-zinc-600 transition-transform duration-300 ${isOpen ? 'rotate-180 text-white' : ''}`} />
            </button>
            ...
            {isOpen && createPortal(
                <div
                    ...
                    className={`
                        p-1 sm:p-1.5 bg-[#0A0A0A] border border-white/10 rounded-xl sm:rounded-2xl shadow-2xl
                ...
                >
                    {options.map((option) => (
                        <button
                            ...
                            className={`w-full flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3.5 rounded-lg sm:rounded-xl transition-all text-xs sm:text-sm font-bold active:scale-[0.98] ${option.value == value ? 'bg-white/10 text-white' : 'text-zinc-400 hover:text-white hover:bg-white/5'}`}
                        >
                            {option.icon && <option.icon className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${option.value == value ? 'text-white' : 'text-zinc-600'}`} />}
                            <span className="flex-1 text-left">{option.label}</span>
                            {option.value == value && <Check className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-blue-400" />}
                        </button>
                    ))}
                </div>
                , document.body)}
